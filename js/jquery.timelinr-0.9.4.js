...
jQuery.fn.timelinr = function(options){
	...
	$(function(){
		...
		$(settings.datesDiv+' a').click(function(event){
			...
			$(settings.issuesDiv+' li').fadeTo(settings.issuesSpeed, settings.issuesTransparency, function(){$(this).removeAttr('filter')}).removeClass(settings.issuesSelectedClass).eq(currentIndex).addClass(settings.issuesSelectedClass).fadeTo(settings.issuesTransparencySpeed,1, function(){$(this).removeAttr('filter')});
			...
			if(settings.orientation == 'horizontal') {
				...
				if(currentPositionIssues <= -(widthIssue*howManyIssues-(widthIssue))) {
					...
					if (!$(settings.issuesDiv).is(':animated')) {
						...
						$(settings.issuesDiv+' li').fadeTo(settings.issuesSpeed, settings.issuesTransparency, function(){$(this).removeAttr('filter')});
						$(settings.issuesDiv+' li.'+settings.issuesSelectedClass).removeClass(settings.issuesSelectedClass).next().fadeTo(settings.issuesTransparencySpeed, 1, function(){$(this).removeAttr('filter')}).addClass(settings.issuesSelectedClass);
			...
			if(settings.orientation == 'horizontal') {
				...
				if(currentPositionIssues >= 0) {
					...
					if (!$(settings.issuesDiv).is(':animated')) {
						$(settings.issuesDiv).animate({'marginLeft':currentPositionIssues+widthIssue},{queue:false, duration:settings.issuesSpeed});
						$(settings.issuesDiv+' li').fadeTo(settings.issuesSpeed, settings.issuesTransparency, function(){$(this).removeAttr('filter')});
						$(settings.issuesDiv+' li.'+settings.issuesSelectedClass).removeClass(settings.issuesSelectedClass).prev().fadeTo(settings.issuesTransparencySpeed, 1, function(){$(this).removeAttr('filter')}).addClass(settings.issuesSelectedClass);
						$(settings.datesDiv).animate({'marginLeft':currentIssueDate},{queue:false, duration:settings.datesSpeed});
